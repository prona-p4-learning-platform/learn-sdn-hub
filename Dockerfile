...
LABEL de.hs-fulda.netlab.name="prona/learn-sdn-hub" \
      ...
      de.hs-fulda.netlab.docker.cmd="docker run -it --rm -p 3001:3001 prona/learn-sdn-hub 127.0.0.1 3001 127.0.0.1 22 p4 p4"
...
RUN echo '#!/bin/bash\n' \
         'if [ ! $# -eq 4 ]\n' \
         'then\n' \
         '  echo "You need to specify a host used to run the P4 assignments.\nThis can be done, e.g., by running \"docker run -it --rm -p 3001:3001 prona/learn-sdn-hub <BACKEND_IP_ADDRESS> <BACKEND_HTTP_PORT> <VBOX_IP_ADDRESS> <VBOX_SSH_PORT> <SSH_USERNAME> <SSH_PASSWORD>\"\n(see also VBOX_IP_ADDRESSES, VBOX_SSH_PORTS, SSH_USERNAME and SSH_PASSWORD in learn-sdn-hub documentation)"\n' \
         '  exit 1\n' \
         'fi\n' \
         'export BACKEND_IP_ADDRESS=$1\n' \
