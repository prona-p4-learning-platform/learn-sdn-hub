...
You can run assignments in a Kubernetes environment. A `kubeconfig` file can be generated by the endpoint in the Kubernetes cluster and this file is than mounted in the assignment container/VM. Simply add the `mountKubeconfig` field to the assignment configuration.
...
```sh
K8S_MGMT_SVC_IP="xxx.xxx.xxx.xxx" # IP of the Kubernetes management service
K8S_MGMT_PASSWORD="..." # SSH Password for the Kubernetes management service
K8S_CLUSTER_IP="https://xxx.xxx.xxx.xxx:6443" # Cluster IP that is used in the kubeconfig file
K8S_CERT_AUTH_DATA="LS0tLS1CRUd..." # The certificate authority data (base64 encoded) that is used in the kubeconfig file
KUBECTL_STORE_PATH="/tmp" # The path in the backend where the kubeconfig file is stored
```