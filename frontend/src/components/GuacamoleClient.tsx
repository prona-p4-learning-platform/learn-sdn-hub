...
export default class GuacamoleClient extends Component<GuacamoleClientProps> {
  ...
  componentDidMount(): void {
    ...
    const keyboard = new Guacamole.Keyboard(input!);
    ...
    this.guacaClient.onerror = (error) => {
      if (error.code === Guacamole.Status.Code.UPSTREAM_NOT_FOUND) {
        ...
        this.showErrorMessage(
          "Upstream server not found. Please contact an admin.",
        );
      } else {
        this.showErrorMessage(
  ...
  render(): JSX.Element {
    return (
      <>
        ...
        <input type="text" id="guacInput"></input>
        <p id="guacErrorMessage"></p>
        <div id="guacLoading">
          <CircularProgress />
        </div>
      </>
